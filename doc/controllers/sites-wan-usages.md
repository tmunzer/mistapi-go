...
| `peerMac` | `*string` | Query, Optional | Filter results by peer MAC address |
| `portId` | `*string` | Query, Optional | Port ID for the device |
| `peerPortId` | `*string` | Query, Optional | Peer Port ID for the device |
| `policy` | `*string` | Query, Optional | Filter results by WAN path policy |
| `tenant` | `*string` | Query, Optional | Filter results by tenant network |
| `pathType` | `*string` | Query, Optional | Filter results by port path type |
| `distinct` | [`*models.WanUsagesCountDistinctEnum`](../../doc/models/wan-usages-count-distinct-enum.md) | Query, Optional | Field used to group this count response. enum: `mac`, `path_type`, `peer_mac`, `peer_port_id`, `policy`, `port_id`, `tenant`<br><br>**Default**: `"policy"` |

...
Count BGP peer statistics for a site, optionally grouped by the `distinct` field and filtered by peer state. Use [Count Org BGP Stats](../../doc/controllers/orgs-stats-bgp-peers.md#count-org-bgp-stats) to count BGP peer statistics across the organization.
...
| `state` | `*string` | Query, Optional | Filter peer results by state |
...
Search BGP peer statistics for a site with filters for device, neighbor, VRF, and time range. Use [Search Org BGP Stats](../../doc/controllers/orgs-stats-bgp-peers.md#search-org-bgp-stats) to search BGP peer statistics across the organization.
...
| `neighborMac` | `*string` | Query, Optional | Filter peer results by neighbor MAC address |
...
apiResponse, err := sitesStatsBGPPeers.SearchSiteBgpStats(ctx, siteId, nil, nil, nil, &limit, nil, nil, &duration, &sort, nil)
if err != nil {
    switch typedErr := err.(type) {
        case *errors.ResponseHttp400:
            log.Fatalln("ResponseHttp400Exception: ", typedErr)

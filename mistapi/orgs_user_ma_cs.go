...
func (o *OrgsUserMACs) UpdateOrgMultipleUserMacs(
	...
	error) {
	req := o.prepareRequest(ctx, "PUT", "/api/v1/orgs/%v/usermacs")
	req.AppendTemplateParams(orgId)
	req.Authenticate(
		NewOrAuth(
			NewAuth("apiToken"),
	...
	var result models.UserMacsUpdate
	decoder, resp, err := req.CallAsJson()
	if err != nil {
		return models.NewApiResponse(result, resp), err
	}

	result, err = utilities.DecodeResults[models.UserMacsUpdate](decoder)
	return models.NewApiResponse(result, resp), err
}

// CountOrgUserMacs takes context, orgId, distinct, limit, start, end as parameters and
// returns an models.ApiResponse with models.UserMacsCount data and
// an error if there was an issue with the request or response.
...
func (o *OrgsUserMACs) CountOrgUserMacs(
	...
	error) {
	req := o.prepareRequest(ctx, "GET", "/api/v1/orgs/%v/usermacs/count")
	req.AppendTemplateParams(orgId)
	req.Authenticate(
		NewOrAuth(
			NewAuth("apiToken"),
	...
	var result models.UserMacsCount
	decoder, resp, err := req.CallAsJson()
	if err != nil {
		return models.NewApiResponse(result, resp), err
	}

	result, err = utilities.DecodeResults[models.UserMacsCount](decoder)
	return models.NewApiResponse(result, resp), err
}

// DeleteOrgMultipleUserMacs takes context, orgId, body as parameters and
...
func (o *OrgsUserMACs) DeleteOrgMultipleUserMacs(
	...
	*http.Response,
	error) {
	req := o.prepareRequest(ctx, "POST", "/api/v1/orgs/%v/usermacs/delete")
	req.AppendTemplateParams(orgId)
	req.Authenticate(
		NewOrAuth(
			NewAuth("apiToken"),

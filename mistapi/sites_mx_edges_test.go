...
// TestSitesMxEdgesTestCreateSiteMxEdge tests the behavior of the SitesMxEdges
func TestSitesMxEdgesTestCreateSiteMxEdge(t *testing.T) {
	ctx := context.Background()
	siteId, errUUID := uuid.Parse("000000ab-00ab-00ab-00ab-0000000000ab")
	if errUUID != nil {
		t.Error(errUUID)
	...
	apiResponse, err := sitesMxEdges.CreateSiteMxEdge(ctx, siteId, nil)
	if err != nil {
		t.Errorf("Endpoint call failed: %v", err)
	}
	testHelper.CheckResponseStatusCode(t, apiResponse.Response.StatusCode, 200)

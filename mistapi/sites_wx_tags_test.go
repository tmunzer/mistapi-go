...
// TestSitesWxTagsTestGetSiteCurrentMatchingClientsOfAWxTag tests the behavior of the SitesWxTags
func TestSitesWxTagsTestGetSiteCurrentMatchingClientsOfAWxTag(t *testing.T) {
    ctx := context.Background()
    siteId, errUUID := uuid.Parse("000000ab-00ab-00ab-00ab-0000000000ab")
    if errUUID != nil {
        t.Error(errUUID)
    ...
    testHelper.CheckResponseHeaders(t, apiResponse.Response.Header, expectedHeaders, true)
    expected := `[{"mac":"5684dae9ac8b","since":1428939600}]`
    testHelper.KeysBodyMatcher(t, expected, apiResponse.Response.Body, false, false)
}

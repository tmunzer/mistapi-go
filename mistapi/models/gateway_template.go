...
func (g GatewayTemplate) toMap() map[string]any {
	...
	if g.GatewayMgmt != nil {
		structMap["gateway_mgmt"] = g.GatewayMgmt.toMap()
	}
	if g.Id != nil {
		structMap["id"] = g.Id
	}
	...
	if g.UrlFilteringDenyMsg != nil {
		structMap["url_filtering_deny_msg"] = g.UrlFilteringDenyMsg
	}
	if g.VrfConfig != nil {
...
func (g *GatewayTemplate) UnmarshalJSON(input []byte) error {
	...
	g.GatewayMgmt = temp.GatewayMgmt
	g.Id = temp.Id
	...
	g.Type = temp.Type
	g.UrlFilteringDenyMsg = temp.UrlFilteringDenyMsg
